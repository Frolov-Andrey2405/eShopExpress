...
class ProductViewTest(TestCase):
    def test_get_products(self):
        ...
        product_1 = Product.objects.create(
            title='Product 1', category=category,
            image=uploaded, slug='product-1'
        )
        product_2 = Product.objects.create(
            title='Product 2', category=category,
            image=uploaded, slug='product-2'
        )
...
class ProductDetailViewTest(TestCase):
    def test_get_product_by_slug(self):
        ...
        product = Product.objects.create(
            title='Product 1', category=category,
            slug='product-1', image=uploaded
        )

        response = self.client.get(reverse(
            'shop:product-detail',
            kwargs={'slug': 'product-1'}
        ))
...
class CategoryListViewTest(TestCase):
    ...
    def test_status_code(self):
        response = self.client.get(reverse(
            'shop:category-list', args=[self.category.slug]
        ))
        self.assertEqual(response.status_code, 200)
